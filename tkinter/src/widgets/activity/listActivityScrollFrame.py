...
class ActivityScrollFrame(ctk.CTkScrollableFrame):
    ...
    def add_item(self,culinary_destination):
        ...
        frame_item.minimal_price_label = ctk.CTkLabel(frame_item,text="Precio minimo: $ "+str(culinary_destination.minimal_price), fg_color="transparent",text_color=color.TEXT)
        frame_item.minimal_price_label.grid(row=3,column=0,padx=10,pady=(5,5),sticky="w")

        frame_item.maximum_price_label = ctk.CTkLabel(frame_item,text="Precio maximo: $ "+str(culinary_destination.maximum_price), fg_color="transparent",text_color=color.TEXT)
